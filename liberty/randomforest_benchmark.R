...
cat("Reading data\n")
train <- read_csv('~/Dropbox/kaggle/liberty/train.csv')
...
cat("Training model\n")
rf <- randomForest(trainFea[,3:34], trainFea$Hazard, ntree=100, imp=TRUE, do.trace=TRUE)

cat("Making predictions\n")
submission <- data.frame(Id=test$Id)
submission$Hazard <- predict(rf, extractFeatures(testFea[,2:33]))
write_csv(submission, "~/Dropbox/kaggle/liberty/1_random_forest_benchmark.csv")
...
ggsave("~/Dropbox/kaggle/liberty/2_feature_importance.png", p, height=12, width=8, units="in")
